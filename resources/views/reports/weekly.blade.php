...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <form action="{{ route('reports.weekly') }}" method="GET" class="flex flex-wrap items-center gap-4">
            <div>
                ...
                <input 
                    ...
                    class="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent"
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('reports.weekly.pdf', ['week_start' => $weekStart->format('Y-m-d')]) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-rose-500 text-white rounded-lg hover:bg-rose-600 transition-colors">
    ...
    <div class="bg-white rounded-xl border border-slate-200">
        <div class="p-6 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800">Orders: {{ $weekStart->format('M d') }} - {{ $weekEnd->format('M d, Y') }}</h3>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($orders as $order)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 text-slate-600 text-sm">{{ $order->items->pluck('service.name')->join(', ') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-slate-400">No orders for this week</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
