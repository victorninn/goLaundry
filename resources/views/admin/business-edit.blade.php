...
@section('page-description', 'Update your business information')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-slate-200 p-6">
        <form action="{{ route('business.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            @if($business->logo)
                <div class="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                    <img src="{{ $business->logo_url }}" alt="Logo" class="w-16 h-16 object-contain rounded-lg border border-slate-200">
                    <div>
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        value="{{ old('business_registration_number', $business->business_registration_number) }}"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none"
                    >
            ...
            <div>
                <label for="logo" class="block text-sm font-medium text-slate-700 mb-2">Logo {{ $business->logo ? '(Replace)' : '(Optional)' }}</label>
